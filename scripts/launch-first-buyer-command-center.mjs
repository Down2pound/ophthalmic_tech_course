...
const recommendedReportPath = "launch-evidence/first-buyer-command-center.md";
...
function renderFirstBuyerCommandCenter() {
  return [
    ...
    "pnpm launch:manual-fulfillment",
    ...
    linkLine(
      "Free preview",
      previewUrl,
      "Safe when owner go/no-go says public preview is GO."
    ),
    linkLine(
      "First-buyer overview",
      firstSaleUrl,
      "Safe when owner go/no-go says public preview is GO."
    ),
    linkLine(
    ...
    linkLine(
      "Buyer guide",
      buyerGuideUrl,
      "Safe when someone asks what this is for or whether it fits them."
    ),
    linkLine(
      "Policies",
      policiesUrl,
      "Safe when someone asks about refunds, limits, certification, or supervision."
    ),
    linkLine(
      "Practice packs",
      practicePacksUrl,
      "Use as review-only until practice outreach is GO."
    ),
    linkLine(
      "Individual checkout or learner interest",
      individualCheckoutUrl,
      "Use for interest-list collection before paid checkout is GO. Do not request payment until the right payment path is approved."
    ),
    linkLine(
      "Checkout availability API",
      availabilityUrl,
      "Use for your own verification, not buyer outreach."
    ),
    linkLine(
    ...
    "7. If automated checkout is not ready but an approved first buyer will use a Stripe Payment Link, run `pnpm launch:manual-payment-links` first, then run `pnpm launch:manual-fulfillment` after Stripe shows paid. That packet prepares the protected `/api/support/manual-payment-fulfillments` request.",
...
async function main() {
  const packet = renderFirstBuyerCommandCenter();

  console.log(packet);

  if (process.env.LAUNCH_FIRST_BUYER_REPORT_PATH) {
    const reportPath = path.resolve(process.env.LAUNCH_FIRST_BUYER_REPORT_PATH);
...
main().catch(error => {
  console.error(
    error instanceof Error
      ? error.message
      : "First buyer command center could not be created."
  );
  process.exitCode = 1;
});
