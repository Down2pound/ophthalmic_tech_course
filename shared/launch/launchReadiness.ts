...
export const launchReadinessChecklist: LaunchReadinessItem[] = [
  ...
  },
  {
    ...
    status: "blocked",
    evidence:
      "A server-side entitlement rule can derive access from verified purchases, the webhook can create individual enrollments and practice seat packs, a protected practice-seat manager page can list packs and create learner enrollments without exceeding paid seats, PostgreSQL commerce repositories exist for durable purchases and enrollments, PostgreSQL auth repositories exist for durable magic links and sessions when DATABASE_URL is configured, magic-link request/token helpers exist, a safe passwordless sign-in request route stores hashed magic-link records server-side and sends through a configured transactional email endpoint, a callback route consumes one-time links into HTTP-only session cookies, a session access endpoint checks active server-side enrollments, Module 1 lesson bodies are served through a protected lesson endpoint, Module 1 lesson completion and assessment attempts use PostgreSQL when DATABASE_URL is configured, certificate eligibility is checked server-side from durable lesson and quiz progress, a db:setup command applies the required schemas, and runtime readiness checks auth environment setup, but schemas still need to be run and verified in deployment before full production sale.",
    nextAction:
  ...
  },
  {
    ...
    status: "in-progress",
    evidence:
      "Module 1 has a protected server-side knowledge-check endpoint that returns questions without answer keys, a protected submit endpoint that scores answers server-side, a server-side attempt store that tracks quiz progress, a PostgreSQL assessment repository for durable attempts when DATABASE_URL is configured, certificate eligibility that requires a passed server-scored knowledge check, a PostgreSQL-ready assessment schema, and a db:setup command, but the schema still needs to be run in production and older client quiz data plus future module assessments still need migration before high-stakes completion logic.",
    nextAction:
