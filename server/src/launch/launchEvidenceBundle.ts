...
function renderReadme({
  ...
  return [
    ...
    "- `first-week-sales-plan.md`: seven-day controlled outreach plan for early buyers.",
...
export async function createLaunchEvidenceBundle({
  ...
  const firstWeekSalesPlan = await readFile(
    path.resolve(projectRoot, "docs/launch/first-week-sales-plan.md"),
    "utf8"
  );
  ...
  const staticFirstSalePageGuide = await readFile(
    path.resolve(projectRoot, "docs/launch/static-first-sale-page-guide.md"),
    "utf8"
  );
  ...
  await writeFile(
    path.join(resolvedOutputDir, "first-week-sales-plan.md"),
    firstWeekSalesPlan
  );
  await writeFile(
  ...
  await writeFile(
    path.join(resolvedOutputDir, "static-first-sale-page-guide.md"),
    staticFirstSalePageGuide
  );
  await writeFile(
